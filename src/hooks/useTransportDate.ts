...
export function useTransportDate(): UseTransportDate {
  ...
  useEffect(() => {
    ...
    if (!promoteTo && !urlDate) return; /* 何もする必要なし（フォールバックで解決済み） */

    const params = new URLSearchParams(searchParams.toString());
    if (promoteTo) params.set('date', promoteTo);
  ...
  const setDate = useCallback(
    (newDate: string) => {
      ...
      /* 月が変わる場合は ?month も一緒に更新（DateStepper の ⟪/⟫ は月跨ぎを期待） */
      if (newMonthStr !== monthStr) {
        params.set('month', newMonthStr);
      }
      router.replace(`${pathname}?${params.toString()}`, { scroll: false });
    },
    [monthStr, pathname, router, searchParams],
  );
