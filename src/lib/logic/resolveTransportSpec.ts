 ...
 * Phase 30: マーク識別を「emoji+name 文字列」から「AreaLabel.id」に移行。
 *
 * schedule_entry から送迎に必要な情報（エリア・時刻・場所）を
 * 迎/送それぞれで解決する。解決順:
 *   (a) pickup_mark / dropoff_mark（id）が有効 → テナント pickup_areas / dropoff_areas
...
export type TransportSpec = {
  /** AreaLabel.id（マーク解決の正準キー）。null=該当なし */
  areaId: string | null;
  /** "🏠 藤江" のような emoji+name の表示文字列。null=該当なし */
...
/** AreaLabel から表示用ラベル `${emoji} ${name}` を組み立てる */
 ...
 * 同じ id は児童専用側を優先（time/address の上書き）。
 ...
 *   4. 該当なし → null
 *
 * @returns 推論された AreaLabel.id、見つからなければ null
 */
export function inferMarkFromTime(
  ...
  /* マーク候補 id → AreaLabel に解決（areas に無い id = 幽霊 は除外） */
...
export function resolveEntryTransportSpec(
  ...
  /* mark 解決: entry.pickup_mark / dropoff_mark（id）を最優先、無ければ児童マーク × 時刻から推論 */
  const pickupMarkId =
    entry.pickup_mark
